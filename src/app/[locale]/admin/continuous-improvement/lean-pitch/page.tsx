...
const perspectives = [
  {
    ...
    secondObjection: "\"$9/ay çok mü yüksek?\" → Kaçan bir lead ortalama 1-3 saat iş demek. Saatlik ücretini 370 TL'nin üzerinde tutuyorsan, bir lead'in maliyeti aylık aboneliği katlıyor.",
  },
  {
    id: 'investor',
    ...
    secondObjection: '"Tek kurucu riski?" → En büyük kırılma noktası bu — ve kabul edildi. Faz 3 (Beiwe) modüler tasarlandı; gecikmesi lansmanı bloklemiyor. Aşama 2\'de ekip büyümesi planlandı.',
  },
  {
    id: 'advisor',
    ...
    solution: 'Süreç: 20 problem görüşmesi → ücretli 10 kişilik pilot ($9/ay, manuel tahsilat — ücretsiz deneme yok) → ölçüm → build/pivot/devam kararı. Şu ana kadar doğrulanan: prompt caching maliyet testi ($0,045/kredi), kredi oranı gerçek ölçüm (1:4.6:5.6, beklenti 1:3:10\'du). Doğrulanmamış: viral imza dönüşüm oranı, dolar-sabit fiyata karşı kur-churn etkisi.',
  ...
  },
  {
    ...
    secondObjection: '"Neden şimdi?" → Türkiye\'de sosyal ticaretin %67\'si DM üzerinden. WhatsApp Business API kurulumu artık daha erişilebilir. Zamanlamayı bekleyen pazar olgunluğu var.',
  },
];
...
const slides = [
  {
    ...
    icon: Rocket,
    content: '"Bio sayfan uyurken çalışsın." — Sosyal medya üzerinden iş yapan hizmet sahiplerinin kaçırdığı lead\'leri yakalayan, 7/24 müşteri karşılayan konuşan bio platformu.',
  ...
  },
  {
    ...
    risk: '%5 ödeme kapasitesi varsayımı; gerçek oran %3\'e düşerse TR tavanı ~$180K\'ya iner',
  },
  {
    num: 4,
    title: 'Benzersiz Değer Teklifi (UVP)',
    icon: Target,
    content: '"Sosyal medya sayfan uyurken çalışsın — 10 dakikada kur, 7/24 müşteri karşıla." Rakiplerden fark: Linktree link paylaşır (cevaplamaz), CRM\'ler karmaşık (teknik bilgi ister). talkinbio ikisi arasındaki "konuşan bio" katmanını açıyor.',
  ...
  },
  {
    ...
    detail: 'v1 kapsamı: web widget, Beiwe kurulum akışı, lead listesi, temel analytics. v2 kapsamı (Faz 7+): WA Business API + IG DM, çok dilli destek, MENA/LatAm lokalizasyon. Saule imzası viral döngünün motoru: her konuşma bir organik tanıtım. Birim maliyet: $0,026/mesaj (gerçek ölçüm). Kredi oranı: Saule:güncelleme:kurulum = 1:4.6:5.6 (beklenti 1:3:10\'du — gerçek ölçümle revize edildi).',
  ...
  },
  {
    ...
    detail: 'Teorik blended ARPU: $22 (%60×$9 + %30×$29 + %10×$79 = $22,3). Efektif ARPU: $15 — Starter-ağırlıklı erken dönem karması + %20 yıllık indirim. Birim ekonomi: LTV = $15/0,05 = $300 (%5 churn). CAC < $100 hedefi (LTV/3). Starter marjı: %69 (tipik kullanım) → en dar senaryo (yalnızca güncelleme): %11. Kur riski: $9/ay bugün ~370 TL; devalüasyonda TL karşılığı yükselir → churn baskısı. Yanıt: yıllık kilit + değer iletişimi "kaçan lead\'in maliyeti" üzerinden.',
    maurya: '"Fiyat değer hipotezinin testidir. Ücretsiz deneme verirsek taahhüt ölçemeyiz. İlk 10 müşteri öder — bu bizim fiyat doğrulamamız."',
  ...
  },
  {
    ...
    icon: BarChart2,
    content: 'TR adreslenebilir: ~30K satıcı × $15 ARPU = $450K/yıl teorik tavan. TR ARR senaryoları: Kötümser %1 = ~$54K · Orta %3 = ~$162K · İyimser TR tavanı %7 = ~$378K. v2 Global (baz %3): ~4.500 müşteri = $810K ARR. v2 Stretch (%6 yakalama): ~9.000 müşteri = $1,6M ARR.',
    detail: 'Fermi zinciri: 62M TR IG × %2,4 satıcı = 1,5M → × %40 DM kullanan = 600K → × %5 ödeyebilir = 30K. Muhafazakâr not: "%5 ödeme istekliliği" en spekülatif çarpan — $9/ay dolar-sabit, kur bariyeriyle %3\'e düşerse TR tavanı ~$270K. Global: aynı zincir MENA+LatAm için ek ~120K → toplam adreslenebilir ~150K. Pazar için Cal.com benchmarkı: 60K ödeme yapan, $5M ARR — referans tempo.',
    ...
    risk: '%5 ödeme istekliliği %3\'e düşerse TR tavanı $270K\'a iner; tüm senaryolar ~2 kat küçülür',
  },
  {
    num: 8,
    ...
    detail: 'Doğrulanmış: prompt caching testi ($0,045/kredi · $0,026/mesaj), kredi oranı gerçek ölçüm (1:4.6:5.6), manuel tahsilat akışı, Faz 1.8 viral imza canlı. Doğrulanmamış: viral imza dönüşüm oranı (UTM\'lerle ölçülecek), dolar-sabit fiyata kur-churn tepkisi, $9/ay ödeme istekliliği gerçek oran, Beiwe kurulum süresi (10 dk vaadi). Erken sinyal: Uliana Pehlivan test kullanıcısı (tek ücretsiz hesap). Ücretli pilot başlangıcı bekliyor.',
    maurya: '"Çekim, tahmin değil kanıttır. Meraklı kullanıcılar değil, ödeme yapan müşteriler sayılır. Her diğer metrik vanity."',
    keyMetric: 'Doğrulanmış: $0,045/kredi · Doğrulanmamış: viral dönüşüm, kur-churn',
