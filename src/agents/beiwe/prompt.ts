...
export function buildBeiweStaticPrompt({ business, locale }: BuildBeiweStaticPromptParams): string {
  ...
  return `
      ...
      12. DİL EŞİTLEME (syncBlockLanguages): Kullanıcı bir bölümü editörde TEK bir dilde elle düzenleyip sana "şu bölümü şu dilde güncelledim, diğer dillere de eşitle/çevir" derse, o bölümü YENİDEN OLUŞTURMA ve add* araçlarını KULLANMA (onlar item ekler, kopya çıkar) — 'syncBlockLanguages' aracını kullan. Akış: (a) "Mevcut Sayfa Blokları"ndan o bölümün KAYNAK dildeki (kullanıcının düzenlediği dil) güncel metnini oku; (b) yalnızca kullanıcının istediği HEDEF dillere çevir — kaynak dili çeviriye DAHİL ETME, o olduğu gibi kalmalı, asla yeniden yazma/başka kelimelerle ifade etme; (c) liste bloklarında (services/gallery/testimonials/faq) item'ları mevcut SIRAYLA, aynı sayıda eşleştir; (d) 'custom' (kullanıcının eklediği ayrı) bir bölümse, bloğun 'id' alanını 'blockId' olarak gönder. Kullanıcı hangi dilleri istediğini söylemdiyse hedef dilleri sor (varsayılan: kaynak dışındaki tüm aktif diller). 'links' bölümünün çevrilecek metni yoktur, onu eşitleme.
    `;
}
