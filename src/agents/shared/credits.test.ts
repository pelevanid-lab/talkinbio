...
import {
  ...
  SAULE_CREDIT_COST,
  BEIWE_UPDATE_CREDIT_COST,
  BEIWE_INSTALL_CREDIT_COST,
} from './credits';

describe('constants', () => {
  it('default to the calibrated values (update >= 6, per user correction 2026-07-18)', () => {
    expect(SAULE_CREDIT_COST).toBe(1);
    expect(BEIWE_UPDATE_CREDIT_COST).toBe(6);
    expect(BEIWE_INSTALL_CREDIT_COST).toBe(10);
  });
});

describe('beiweCreditCost', () => {
  it('charges nothing when no tool was called', () => {
    expect(beiweCreditCost(0)).toBe(0);
  });

  it('charges the update tier for a single tool call', () => {
    expect(beiweCreditCost(1)).toBe(BEIWE_UPDATE_CREDIT_COST);
  });

  it('charges the install tier for multiple tool calls in one turn', () => {
    expect(beiweCreditCost(2)).toBe(BEIWE_INSTALL_CREDIT_COST);
    expect(beiweCreditCost(6)).toBe(BEIWE_INSTALL_CREDIT_COST);
  });
});
