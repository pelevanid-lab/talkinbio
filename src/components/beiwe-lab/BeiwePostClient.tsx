        ...
        <div className="space-y-5">
          ...
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            ...
            <p className="text-xs text-slate-500 mb-3">
              {t.rich('postTemplateDesc', { strong: (chunks) => <strong>{chunks}</strong> })}
            </p>
              ...
              {POST_TEMPLATES.map((tpl) => (
                ...
                >
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-sm font-semibold text-slate-900">{t(tpl.label as any)}</span>
                    <span className="text-[10px] text-slate-400 whitespace-nowrap">{t(tpl.pillar as any)}</span>
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5 leading-snug">{t(tpl.hint as any)}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          {needsImage && (
            <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <Upload className="w-4 h-4" />{t('postUploadBtn')}</button>
                <button
                ...
                {previousImageUrl && (
                  ...
                  >
                    <Undo2 className="w-3.5 h-3.5" />{t('postRestoreBtn')}</button>
                )}
                {imageUrl && (
                  <button
                    onClick={clearImage}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            ...
            <div ref={previewWrapRef} className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
              ...
              {(texts[locale].headline.trim() || texts[locale].subline.trim()) && (
                ...
                >{t('postDragTextLabel')}</button>
              )}
            </div>
            {needsImage && template.imageMode !== 'card' && !imageUrl && (
              <p className="text-xs text-amber-600 mt-2">
                {t('postPreviewNoImageWarning')}
              </p>
            )}

            <div className="mt-3 pt-3 border-t border-slate-100 space-y-2.5">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-slate-700">{t('postTweakTitle')}</h3>
                ...
                <p className="text-[11px] text-slate-400">
                  Önizlemedeki <strong>{t('postDragImageLabel')}</strong>/<strong>{t('postDragTextLabel')}</strong> tutamaçlarını sürükleyerek konumlarını değiştirebilirsin.
                </p>
              </div>
          </div>
