...
import { useBeiweSuggestions } from './useBeiweSuggestions';

describe('useBeiweSuggestions', () => {
  it('warns when about text is short', () => {
    const blocks = [{ type: 'about', content: { tr: { text: 'Kısa metin' } } }];
    const { result } = renderHook(() => useBeiweSuggestions(blocks, null, null, 'tr', true));
  ...
  it('does not suggest a gallery once one exists', () => {
    ...
    const { result } = renderHook(() => useBeiweSuggestions(blocks, 'photographer', null, 'tr', true));
    expect(result.current.some((s) => s.id === 'missing-gallery')).toBe(false);
  });

  it('suggests testimonials for trust sectors without one', () => {
    const { result } = renderHook(() => useBeiweSuggestions([], 'consultant', null, 'tr', true));
